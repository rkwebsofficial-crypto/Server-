...
if not UPI_ID:
    raise ValueError("UPI_ID missing")

if not REQUIRED_CHANNEL:
    raise ValueError("REQUIRED_CHANNEL missing")
...
async def server_menu(update, context):
    ...
    message = await update.message.reply_text(
        ...
        "Your subscription is checked before "
        "showing this list.",
...
async def delete_server_list(
    ...
    try:
        ...
        await context.bot.send_message(
            ...
            "⌛ Server access list expired after 1 hour."
...
async def support_reply_start(update, context):
    ...
    await query.message.reply_text(
        ...
        f"💬 Send your reply to user {user_id}:"

    )


async def process_support_reply(update, context):
...
async def admin_plans(update, context):
...
async def otp_server_info(update, context):
    ...
    if active:
        ...
        await update.message.reply_text(
            ...
            "Open 🖥 SERVER to view available servers."
...
async def text_handler(update, context):
    ...
    if await process_support_reply(
        update,
        context
    ):
...
async def callback_handler(update, context):
    ...
    elif data.startswith("supportreply_"):
